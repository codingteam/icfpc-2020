...
class Ship:
    ...
    ship_params: List[int] = None
    x5: int = None
    x6: int = None
    x7: int = None

    def __str__(self) -> str:
        return "[{:02d}{} ({:03d}{:+03d},{:03d}{:+03d}) {} {:02d}|{:02d}|{:02d}]".format(
            self.ship_id,
            ...
            self.xy_velocity[1],
            "|".join(["{:02d}".format(x) for x in self.ship_params]),
            self.x5,
            self.x6,
            self.x7,
        )
...
def parse_ship(ship):
    ...
    parsed_ship.ship_params = ship[4]
...
def parse_game_data(game_data):
    ...
    for ship_and_history_by_side in game_data[3][2]:
        for ship in ship_and_history_by_side[:1]:
            parsed_ship = parse_ship(ship)
            sides[parsed_ship.is_defender].append(parsed_ship)

    if len(sides) != 2:
        print("+"*15, "Totally", len(sides), "sides")

    if game_state.we_defend:
